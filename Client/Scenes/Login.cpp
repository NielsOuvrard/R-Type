...
void Login::build()
{
    _children["logo"] = std::make_shared<Image>(_engine, "assets/sprites/haze_logo.png", AxisPair{150.0, 70.0}, AxisPair{0.7, 0.7});
    _children["logo"]->build();

    _children["ip_img"] = std::make_shared<Image>(_engine, "assets/sprites/button.png", AxisPair{210, 276}, AxisPair{1, 1});
    _children["ip_img"]->build();
    _children["ip"] = std::make_shared<TextInput>(_engine, "ip", AxisPair{250, 300});
    _children["ip"]->build();
    get<TextInput>("ip")->setHitbox(-20, -20, 320, 81);

    _children["port_img"] = std::make_shared<Image>(_engine, "assets/sprites/button.png", AxisPair{210, 426}, AxisPair{1, 1});
    _children["port_img"]->build();
    _children["port"] = std::make_shared<TextInput>(_engine, "port", AxisPair{250, 450});
