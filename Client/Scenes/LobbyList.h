...
public:
    LobbyList(Haze::Engine &engine, uint32_t &currentLobby, std::function<void()> join, std::function<void(uint8_t)> create, std::function<void()> disconnect);
...
public:
    ...
    std::function<void(uint8_t)> _create;
